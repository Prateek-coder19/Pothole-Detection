...
if uploaded_files:
    ...
    img_array = results[0].plot()  # Get the plotted image as a numpy array
    img_pil = Image.fromarray(img_array) # for save the image

    st.image(img_pil, caption="Detected Potholes", use_column_width=True)
    

    number_of_potholes = len(results[0].boxes)  # Assuming each box corresponds to a pothole
